...
#'   "came.from": a vector of size four, showing the number of traces that entered this cell from the 
#'       north, east, south, west directions.
#'   "going.to": as with 'came.from', but for traces leaving this cell
...
calc.cell <- function(cell, the.traces, run.gc=FALSE, plot=TRUE) {
  ...
  # Now intersect those traces. For some reason gIntersection sometimes breaks whe the intersecting
...
make.transition.matrix <- function(the.grid, the.traces, plot=TRUE, multithread=TRUE) {
...
Calculate the transition matrix! This will take some time, use loads of memory, and heat up your computer. Good luck.

```{r calculateTrans, eval=FALSE}

result <- make.transition.matrix(grid,all.routes, plot=FALSE, multithread = FALSE)
