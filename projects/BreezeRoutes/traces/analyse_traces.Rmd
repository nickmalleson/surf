...
_The code below is for testing the aggorithm, it defines a few test routes and makes a small grid_
...
The following function `make.transition.matrix` (and its helper functions) do the analysis. The function runs over each cell, finds routes passing through it, and returns a list of cells with the following properties:
...
calc.cell <- function(cell, the.traces, run.gc=FALSE, plot=TRUE) {
  ...
  if (length(which(intersecting))>0) {
    ...
    # lines are passed directly (as below) so instead pass each line individuall in a separate call
    ...
    for (lineno in seq(1,length(lines.in.cell))) {
      ...
      if ( class(lines.in.cell[[lineno]]) == "SpatialLines" ) { #
        ...
        for (line in the.lines) {
          ...
          # XXXX CHANGE THIS - CHECK IF THE LINE STARTS HERE. IF SO, INCREMENT THE going.to COUNTER.
          # Otherwise, just increment the came.from
          ...
          if (!start.within & !end.within) { # PASES THROUGH
            ...
            index.from <- find.nsew(cell,start) # This is a list with four booleans (N, S, E, W).
            index.to <- find.nsew(cell,end)
            #print("**")
            #print(index.from)
            #print(index.to)
            stopifnot( length(index.from[index.from==TRUE]) == 1 ) # Check only one true value in the list
            stopifnot( length(index.to[index.to==TRUE]) == 1 ) 
            
            came.from[c(index.from)] <- came.from[c(index.from)] + 1
            going.to[c(index.to)]  <- going.to[c(index.to)] + 1
...
result.to.spdf <- function(result) {
  ...
  polys.data <- data.frame(
    ...
    "came.from.N" = unlist(lapply(result, function(x){x$came.from[[1]]})),
    "came.from.E" = unlist(lapply(result, function(x){x$came.from[[2]]})),
    "came.from.S" = unlist(lapply(result, function(x){x$came.from[[3]]})),
    "came.from.W" = unlist(lapply(result, function(x){x$came.from[[4]]})),
    "going.to.N" = unlist(lapply(result, function(x){x$going.to[[1]]})),
    "going.to.E" = unlist(lapply(result, function(x){x$going.to[[2]]})),
    "going.to.S" = unlist(lapply(result, function(x){x$going.to[[3]]})),
    "going.to.W" = unlist(lapply(result, function(x){x$going.to[[4]]})),
...
Now mess around with the data a bit to get an actual transition matrix

```{r makeMatrix} 

# Define an empty matrix
trans.mat.1 <- matrix(data=0, nrow=total.cells, ncol=total.cells)

# Iterate over each cell and increment the relevant values
for(i in seq(1,nrow(result1.spdf))) {
  cell <- result1.spdf[i,]
  id <- cell$ID
  x <- cell$CellX
  y <- cell$CellY
  # For each came.from.X and going.to.X increment the appropriate matrix cell
  
  
  
  
  
  trans.mat.1[y-1,x] <- trans.mat.1[y-1,x] + cell$came.from.N
  trans.mat.1[y,x+1] <- trans.mat.1[y,x+1] + cell$came.from.E
  trans.mat.1[y+1,x] <- trans.mat.1[y+1,x] + cell$came.from.S
  trans.mat.1[y,x-1] <- trans.mat.1[y,x-1] + cell$came.from.W
}
