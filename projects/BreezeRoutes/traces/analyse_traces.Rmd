...
find.nsew <- function(the.cell,the.point) {
  ...
  l_points$bearing = c("w", "n", "e", "s") # add names to edges (XXXX CAN GUARANTEE ORDER OF EDGES?)
  #text(x = l_points@coords[,1] + 0.1, y = l_points@coords[,2] + 0.1, l_points$bearing)
  #plot(the.point, add = T)
...
writeOGR(SpatialLinesDataFrame(relative.shortest.routes.oneline, data=data.frame(c(1)) ), dsn="./result", layer="relative_shortest_routes_oneline", driver="ESRI Shapefile" )
writeOGR(SpatialLinesDataFrame(relative.longest.routes.oneline, data=data.frame(c(1)) ), dsn="./result", layer="relative_longest_routes_oneline", driver="ESRI Shapefile" )
...
This results in `r length(relative.longest)` longest paths and `r length(relative.shortest)` non-longest paths (_I don't want to say 'shortest' because a shortest path is the equivalent route that someone could have taken but might not have done).
...
# Finally make a transition matrix 
result1.matrix.longest  <- spdf.to.matrix(result1.longest.spdf)
result1.matrix.shortest <- spdf.to.matrix(result1.shortest.spdf)
...
```{r createWeekdayEndSubsets, fig.width=15, eval=FALSE}
...
writeOGR(SpatialLinesDataFrame(weekday.routes.oneline, data=data.frame(c(1)) ), dsn="./result", layer="weekday_routes_oneline", driver="ESRI Shapefile" )
writeOGR(SpatialLinesDataFrame(weekend.routes.oneline, data=data.frame(c(1)) ), dsn="./result", layer="weekend_routes_oneline", driver="ESRI Shapefile" )
...
#plot(relative.longest, main="longest")
#plot(all.routes[ which( matched.lengths >= (shortest.lengths*0.2)+shortest.lengths)], main="longest")
plot(weekend.routes, main="weekend routes")
plot(study.area, add=T, border="blue")
#plot(relative.shortest, main="not longest")
#plot(all.routes[ which( matched.lengths < (shortest.lengths*0.2)+shortest.lengths) ], main="not longest")
plot(weekday.routes, main="weekday routes")
plot(study.area, add=T, border="blue")
...
Sanity check that this looks sensible

```{r weekdayEnd-sanityCheck}
...
```{r rerunAsymmetryAnalysis-weekdayEnd }
...
av.grid.temp <- readOGR( ds = "./tony_grid/", layer="BOS CAM 200M")
...
# Add ID, CELLX and CELLY columns.
# These are needed so that the functions don't break, but as Tony's grid is square the numbering and cellX/cellY values wont
# be correct. Hopefully this doesn't matter for now? (Not doing this now because Tony's grid isn't square which could be annoying)
#total.av.cells <- nrow(av.grid)
#num.av.cells <- XXXX # What to make this? my code assumes that the grid has an equal number of rows and columns
#av.grid$ID <- -1
#av.grid$CellX <- -1
#av.grid$CellY <- -1
#grid.ids <- matrix(data=0, nrow=num.av.cells, ncol=num.av.cells)
#cellcount <- 1
...
#av.result.matrix <- spdf.to.matrix(av.result.spdf)
...
av.result.percentage.asymmetry <- -1 # TEMPORARILY SO AS NOT TO BREAK THE FOLLOWING MARKDOWN
