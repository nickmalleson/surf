...
no_cores <- 6 # Set the number of cores to use directly (if too many cores then too much memory is consumed)
...
A hook for knitr that prints the execution time of each chunk (useful). From: [https://stackoverflow.com/questions/24595280/timing-for-chunks](https://stackoverflow.com/questions/24595280/timing-for-chunks). Include `timeit = TRUE` in chunks to activate.

```{r setupTimeIt }

chunk.times <- list() # Record the times for each chunk in a named list
...
knitr::knit_hooks$set(timeit = local({
  ...
  function(before, options) {
    if (before) {
      ...
      paste('\nTime for this code chunk (',options$label,'):', as.character(res),'\n', sep="")
      chunk.times <- c(chunk.times, options$label)
    }
  }})
)

# Set thatoption to TRUE for all chunks
...
```{r findBadMatchedPaths, fig.height=20, cache=TRUE }
...
```{r compareLengthsHist1, cache=TRUE }
...
```{r compareLengthsHist2, cache=TRUE }
par(mfrow=c(1,1))
...
```{r compareLengthsBoxPlot, cache=TRUE}
par(mfrow=c(1,1))
...
```{r hausdorffDistancesHist2, cache=TRUE} 
par(mfrow=c(1,1))
...
```{r hausdorffDistances2, cache=TRUE} 
par(mfrow=c(1,1))
...
```{r makeGridForTransitionMatrix, fig.width=15, cache=TRUE}
#' Make a regular grid
#' 
#' @param ncells The number of cells per row (i.e. the resolution)
#' @param study.area.bbox The bounding box that bounds the study area
#' @param plot Whether to plot the grid and its routes or not
#' @return A named list with three items. The first is the grid itself (called 'grid'), the second is the grid IDs (called 'grid.ids'),.
make.grid <- function(ncells, study.area.bbox, plot=TRUE) {
  # The number of cells per row or column
  tot.cells <- ncells ** 2
  
  # Create the grids - adapted from Brunsdon & Comber (2015, p150)
  cell.width <-  (study.area.bbox[1,2] - study.area.bbox[1,1]) / ncells
  cell.height <- (study.area.bbox[2,2] - study.area.bbox[2,1]) / ncells
  #cell.areas <- c(cell.areas, (cell.width * cell.height) ) # Also remember the cell area for later

  centre.x <- study.area.bbox[1,1] + ( cell.width / 2 )
  centre.y <- study.area.bbox[2,1] + ( cell.height / 2 )
  
  # Make the grid
  grd <- GridTopology(
    cellcentre.offset = c(centre.x, centre.y), # No offset, the grid will just cover all the points
    cellsize = c(cell.width, cell.height),
    cells.dim = c(ncells, ncells)
  )

  # Convert it into a polygons data frame, and give each cell it's x and y locations in the matrix (-1 initially)
  the.grid <- SpatialPolygonsDataFrame(
    as.SpatialPolygons.GridTopology(grd),
    data = data.frame("ID"=rep.int(-1,tot.cells),"CellX"=rep.int(-1,tot.cells),"CellY"=rep.int(-1,tot.cells)),
    match.ID = FALSE
  )
  proj4string(the.grid) <- PROJ4STRING
  rm(grd)
  
  # Set the ID properly and keep a record of the (x,y) coordinates and associated ID
  gridids <- matrix(data=0, nrow=ncells, ncol=ncells)
  cellcount <- 1
  for (rowcount in seq(1,ncells)) {
    for (colcount in seq(1,ncells)) {
      the.grid@data[cellcount,]$CellX <- colcount
      the.grid@data[cellcount,]$CellY <- rowcount
      the.grid@data[cellcount,]$ID <- cellcount
      gridids[rowcount,colcount] <- cellcount
  ...
  if (plot) {
    par(mfrow=c(1,1))
    plot(the.grid, border='blue', main=paste("Routes and the grid (num. cells=",ncells,")",sep="") )
    plot(all.routes.oneline, add=T)
  }
  
  return(list(grid = the.grid, grid.ids = gridids))

}
#save.image(file="analyse_traces-temp.RData") # For debugging

num.cells <- 20
g <- make.grid(ncells = num.cells, study.area.bbox = study.area.bb, plot=TRUE)
grid <- g$grid
grid.ids <- g$grid.ids
...
spdf.to.matrix <- function(spdf, gridids, ncells) {
...
result1.matrix <- spdf.to.matrix(result1.spdf, grid.ids, num.cells)
...
#save(result1, result1.spdf, result1.matrix, num.cells, file="results.RData")
...
test.matrix <- spdf.to.matrix(test.spdf, grid2.ids, num.cells)
...
calc.flow <- function(o,d, tot.cells) { # Calculate flow from o to d, checking for boundary errors
    if ( o > tot.cells || o < 1 || d > tot.cells || d < 1) {
      return (0)
    } else {
...
total.num.cells <- num.cells**2
for (cell in seq(1, total.num.cells)) { 
  ...
  fin <- c(
    calc.flow(n,cell,total.num.cells), # N
    calc.flow(e,cell,total.num.cells), # E
    calc.flow(s,cell,total.num.cells), # S
    calc.flow(w,cell,total.num.cells) # W
  )
  fout <- c(
    calc.flow(cell, n,total.num.cells), # N
    calc.flow(cell, e,total.num.cells), # E
    calc.flow(cell, s,total.num.cells), # S
    calc.flow(cell, w,total.num.cells) # W
  )
...
```{r mapTracesPerCell, fig.width=15, cache=TRUE}
...
Begin by doing the calculations

```{r sensitivity.to.resolution, cache=TRUE}

# Iterate over a number of different resolutions
sen.resolutions <- c(1, 5, 10, 15, 20, 30, 50, 100)

# Remember the grids, results, etc. at the different resolutions ('sen' here is short for sensitivity)
sen.grids <- list()
sen.gridids <- list()
sen.results <- list() # Each element will be a list where each element has info. about a cell (see calc.cell for the specific info)
sen.results.spdf <- list() # The SpatialPolygonsDataFrames for each list
sen.results.matrix <- list() # The transition matrices 
sen.results.asymmetry <- c() # Asymmetry values

for (ncells in sen.resolutions) {
  # Create the grid and remember it
  g <- make.grid(ncells = ncells, study.area.bbox = study.area.bb, plot = FALSE)
  temp.grid <- g$grid
  temp.grid.ids <- g$grid.ids
  
  # Do the maths!
  result <- calc.flows(temp.grid, all.routes, plot=FALSE, multithread = TRUE)
  result.spdf <- result.to.spdf(result) 
  result.matrix <- spdf.to.matrix(result.spdf, temp.grid.ids, ncells)
  asymmetry <- calculate.asymmetry(result.matrix)
  print(paste("Calculated asymmetry for grid with ",ncells, "cells per row: ",asymmetry, sep=""))
  
  # Store the results
  #sen.grids <- c(sen.grids, g)
  #sen.grids <- append(sen.grids, g) # Can't use c() because otherwise it doesn't nest the lists properly (g is a list)
  sen.grids <- c(sen.grids, temp.grid)
  sen.gridids <- c(sen.gridids, temp.grid.ids)
  sen.results <- c(sen.results, result)
  sen.results.spdf <- c(sen.results.spdf, result.spdf)
  sen.results.matrix <- c(sen.results.matrix, result.matrix)
  sen.results.asymmetry <- c(sen.results.asymmetry, asymmetry)
  
  # Write out the results
  writeOGR(result1.spdf, dsn = "sensitivity_results", 
           layer=paste("result_spdf-", ncells, sep=""),
           driver="ESRI Shapefile", overwrite_layer = TRUE)
}

# Save the results as well (might be useful)
save(sen.resolutions, sen.grids, sen.gridids, sen.results, sen.results.spdf, sen.results.matrix, sen.results.asymmetry, 
     file="sen_results.RData")


#save.image(file="analyse_traces-temp.RData") # For debugging
```

Now analyse them. Start with a choropleth map of the total flow to check that the results look similar, albeit at different resolutions.

```{r sensitivity.maps.total.flow, fig.width=10, fig.height=20 }

par(mfrow=c(ceiling(length(sen.results.spdf)/2),2))
for (spdf in sen.results.spdf) { # Loop over each spatial data frame
  
  shades <- auto.shading(spdf$total.intersection, n=9, cutter=quantileCuts)
  choropleth(spdf, spdf$total.intersection, shading = shades, main="Total intersecting flows")
...
Now chart the asymetry. First with full axes, then only looking at grids with a sensible resolution (the first couple of resolutions are unrealistic)


```{r sensitivity.maps.asymmetry, fig.width=10, height=7}

# Make vectors for the grid
x1 <- c() # Square area
x2 <- c() # Number of cells per row
y <- c()


for (index in seq(1,length(sen.results.asymmetry))) { # Loop over each resolution
  
  asym <- sen.results.asymmetry[[index]] # The asymetry fo this resolution
  area <- mean(raster::area(sen.grids[[index]]))/(1000**2) # The square area of the cells
  no.cells <- sen.resolutions[[index]]
...
par(mfrow=c(1,2))
plot(x1,y, type='b', main="Asymmetry at different resolutions", xlab="Cell square area (km^2)", ylab="Asymmetry")

plot(x2,y, type='b', main="Asymmetry at different resolutions", xlab="Number of cells per row", ylab="Asymmetry")

rm(x1,x2,y)
```

Repeat but ignore the two lowest resolution grids.

```{r sensitivity.maps.asymmetry2, fig.width=10, height=7}
...
for (index in seq(3,length(sen.results.asymmetry))) { # Loop over each resolution
...
par(mfrow=c(1,2))
plot(x1,y, type='b', main="Asymmetry at different resolutions", xlab="Cell square area (km^2)", ylab="Asymmetry")
abline(v=mean(raster::area(sen.grids[[which(sen.resolutions==num.cells)]]))/(1000**2), col="red", lty=2 )
abline(h=sen.results.asymmetry[[which(sen.resolutions==num.cells)]], col="red", lty=2)

plot(x2,y, type='b', main="Asymmetry at different resolutions", xlab="Number of cells per row", ylab="Asymmetry")
abline(v = num.cells, col="red", lty=2)
abline(h=sen.results.asymmetry[[which(sen.resolutions==num.cells)]], col="red", lty=2)


# Also do a PDF for the paper
pdf(file="resolution_sensitivity.pdf", width = 7, height = 11.69)
par(mfrow=c(2,1))
plot(x1,y, type='b', xlab="Cell square area (km^2)", ylab="Asymmetry")
abline(v=mean(raster::area(sen.grids[[which(sen.resolutions==num.cells)]]))/(1000**2), col="red", lty=2 )
abline(h=sen.results.asymmetry[[which(sen.resolutions==num.cells)]], col="red", lty=2)

plot(x2,y, type='b', xlab="Number of cells per row", ylab="Asymmetry")
abline(v = num.cells, col="red", lty=2)
abline(h=sen.results.asymmetry[[which(sen.resolutions==num.cells)]], col="red", lty=2)
dev.off()
...
## Shortest path choice v.s. symmetry (aka 'deviation')

**NOTE: in the paper this analysis is now termed 'deviation' (i.e. deviation from the shortest path) rather than longest/shortest**
...
```{r createSubsets, fig.width=15, cache=TRUE}
...
Lets look at the actual lengths of these paths. Turns out that they're quite similar (which isn't entirely unexpected, why should paths that deviate more/less from their shortest path equivalent have anything to do with absolute length)?

```{r longestDeviationPath-sanityCheck, fig.width=15}
...
# Calulate the flows into and out of cells. This makes a list with lots of information about each cell
...
# Finally make a transition matrix 
result1.matrix.longest  <- spdf.to.matrix(result1.longest.spdf, grid.ids, num.cells)
result1.matrix.shortest <- spdf.to.matrix(result1.shortest.spdf, grid.ids, num.cells)
...
#save(result1, result1.spdf, result1.matrix, num.cells, 
...
The asymmetry values for longest and shortest paths are:
 ...
 - For *Longest* (i.e. the most deviation): The percentage asymmetry is `r round(result1.percentage.asymmetry.longest,5) * 100`%. Therefore asymmetric flows account for `r round(result1.percentage.asymmetry.longest,2) * 100`% of all flows, or people change their routes `r round(result1.percentage.asymmetry.longest,2) * 100`% of the time.

 - For *Shortest* (i.e. the least deviation): The percentage asymmetry is `r round(result1.percentage.asymmetry.shortest,5) * 100`%. Therefore asymmetric flows account for `r round(result1.percentage.asymmetry.shortest,2) * 100`% of all flows, or people change their routes `r round(result1.percentage.asymmetry.shortest,2) * 100`% of the time.
...
# Finally make a transition matrix 
result1.matrix.weekday <- spdf.to.matrix(result1.weekday.spdf, grid.ids, num.cells)
result1.matrix.weekend <- spdf.to.matrix(result1.weekend.spdf, grid.ids, num.cells)
...
Now the analysis moves on from comparing deviations from the shortest possible path, to comparing the asymmetry assocated with **absolute path length**. The hypothesis here is that route length might impact on pedestrian decisions.  The rationale is that we might know quite well our neighborhood, so that we take shortest path for short walks, while we might significantly deviate from shortest path for longer routes where we haven’t full knowledge of the environment. Or the opposite of course! (We have less confidence on long routes through unknown neighbourhoods so always stay on the same path).
...
Sanity check: make sure that the subsets exhibit substantially different path lengths:

```{r absolutelongestPath-sanityCheck, fig.width=10, height=7}

# First the longest:

absolute.longest.lengths <- unlist(mclapply(absolute.longest, gLength ))
summary(absolute.longest.lengths)

# Now the shortest:
absolute.shortest.lengths <- unlist(mclapply(absolute.shortest, gLength ))
summary(absolute.shortest.lengths)

# And a histogram
par(mfrow=c(1,2))
hist(absolute.longest.lengths, main="Absolute Longest Path Lengths", breaks="Scott")
hist(absolute.shortest.lengths, main="Absolute Shortest Path Lengths", breaks="Scott")

# And one for the paper
#pdf(file="absolute_length_distribution.pdf", width = 7, height = 11.69)
#par(mfrow=c(2,1))
#hist(absolute.longest.lengths, main="Absolute Longest Path Lengths", breaks="Scott")
#hist(absolute.shortest.lengths, main="Absolute Shortest Path Lengths", breaks="Scott")
#dev.off()
...
# So as not to confuse things later:
rm(absolute.longest.lengths, absolute.shortest.lengths)
...
# Finally make a transition matrix 
result1.matrix.absolute.longest  <- spdf.to.matrix(result1.absolute.longest.spdf,  grid.ids, num.cells)
result1.matrix.absolute.shortest <- spdf.to.matrix(result1.absolute.shortest.spdf, grid.ids, num.cells)
...
Asymettry values:
 ...
 - Asymmetry for absolute **longest** routes: `r result1.percentage.asymmetry.absolute.longest`

 - Asymmetry for absolute **shortest** routes: `r result1.percentage.asymmetry.absolute.shortest`
...
```{r AV.asymmetry.analysis, eval=FALSE }
...
av.result.matrix <- spdf.to.matrix(av.result.spdf, av.grid.ids, num.cells)
...
Here is the amount of time that each chunk took to execute:

```{r reportChunkRuntime}
