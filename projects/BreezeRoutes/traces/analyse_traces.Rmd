...
```{r sensitivity.to.resolution, cache=FALSE}
...
## Absolute path length v.s. symmetry

In the previous section we looked at the relationship between asymmetry and the amount of deviation from the shortest possible path. In this section we will see whether this relationship holds with the _absolute_ path length. I.e. does the degree of asymmetry change in relation to the overall length of a route?

We follow the same methodology as the previous section, although this time routes are segregated into two groups (absolute longest and shortest) depending on their absolute length.
