...
for (rowcount in seq(1,num.cells)) {
  for (colcount in seq(1,num.cells)) {
    ...
    grid.ids[colcount,rowcount] <- cellcount
...
for (rowcount in seq(1,num.cells)) {
  for (colcount in seq(1,num.cells)) {
    ...
    grid2.ids[colcount,rowcount] <- cellcount
    cellcount <- cellcount + 1
  }
}
...
spdf.to.matrix <- function(spdf) {
  # Define an empty matrix. Origins the columns and destinations are rows. 
  ...
  for(i in seq(1,nrow(spdf))) {
    ...
    if ( y > 0 ) { # N
      ...
      did.N <- grid.ids[x,y-1] # Destination ID to the north
      trans.mat[oid,did.N] <- trans.mat[oid,did.N] + cell$departure.count.N
    }
    if ( x < num.cells ) { # E
      did.E <- grid.ids[x+1,y] # Destination ID to the east
      trans.mat[oid,did.E] <- trans.mat[oid,did.E] + cell$departure.count.E
    }
    if ( y < num.cells ) { # S
      did.S <- grid.ids[y+1,x] # Destination ID to the south
      trans.mat[oid,did.S] <- trans.mat[oid,did.S] + cell$departure.count.S
    }
    if ( x > 0 ) { # W
      did.W <- grid.ids[x-1,y] # Destination ID to the west
      trans.mat[oid,did.W] <- trans.mat[oid,did.W] + cell$departure.count.W
    }
  }
  return(trans.mat)
}
