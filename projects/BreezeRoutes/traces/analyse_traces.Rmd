...
It will be interesting to see if there is some relationship between the choice to take the shortest path and the path symmetry. I.e. is the chioce to deviate from the shortest path related in some way to route asymmetry? Or are the chioces independent. Explore this as follows:
 ...
 1. Subset the **longest traces relative to their shortest path** (i.e. those which are at least 20% longer than their shortest). **There are `r length(which((shortest.lengths*0.2)+shortest.lengths>matched.lengths))` / `r length(shortest.lengths)`** of these.
 ...
 1. **Repeat using the longest relative path routes**. 


```{r createSubsets}

# Create a subset of the longest traces relative to their shortest paths (all of those that are 20% longer than their shortest equivalent)

relative.shortest.indices <- which( (shortest.lengths*0.2)+shortest.lengths > matched.lengths)
relative.longest.indices <-  which( (shortest.lengths*0.2)+shortest.lengths <= matched.lengths)
...
par(mfrow=c(1,2))
plot(relative.shortest)
plot(study.area, add=T, border="blue")
plot(relative.longest)
plot(study.area, add=T, border="blue")


```

Now re-do the asymmetry analysis but with these shortest path subsets

```{r rerunAsymmetryAnalysis}
...
# Anlnymise and Save Workspace

It is useful to save all these data so that it isn't necessary to re-run everything.
