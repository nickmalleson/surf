...
for (activity in all.acts) {
  if (i==1) { # First plot
    ...
    plot(acts[acts$Activity==activity,]$date, acts[acts$Activity==activity,]$Intensity, 
         type='o', col=i, ylim=c(0,max(acts$Intensity)), pch=1,
         xlab = "Time", ylab="TotalIntensity")
  } else { # Now for the remaining activities
    points(acts[acts$Activity==activity,]$date, acts[acts$Activity==activity,]$Intensity, type='o', col=i )
  }
...
for (activity in all.acts) {
  if (i==1) { # First plot
    ...
    points(acts[acts$Activity==activity,]$date, acts[acts$Activity==activity,]$BackgroundIntensity, type='o', col=i, pch=2)
    # Time intensity
    points(acts[acts$Activity==activity,]$date, acts[acts$Activity==activity,]$TimeIntensity, type='o', col=i, pch=3)
  } else { # Now for the remaining activities
    points(acts[acts$Activity==activity,]$date, acts[acts$Activity==activity,]$Intensity, type='o', col=i, pch=1)
    points(acts[acts$Activity==activity,]$date, acts[acts$Activity==activity,]$BackgroundIntensity, type='o', col=i, pch=2)
    points(acts[acts$Activity==activity,]$date, acts[acts$Activity==activity,]$TimeIntensity, type='o', col=i, pch=3)
  }
  i <- i + 1 # increment the colour counter (one colour per activity)
...
for (activity in all.acts) {
  ...
  plot(acts[acts$Activity==activity,]$date, acts[acts$Activity==activity,]$Intensity, 
       type='o', col=i, ylim=c(0,max(acts$Intensity)), pch=1,
       xlab = "Time", ylab="Intensity", main=activity)
  # Backgruound intesity
  points(acts[acts$Activity==activity,]$date, acts[acts$Activity==activity,]$BackgroundIntensity, type='o', col=i, pch=2)
  # Time intensity
  points(acts[acts$Activity==activity,]$date, acts[acts$Activity==activity,]$TimeIntensity, type='o', col=i, pch=3)
