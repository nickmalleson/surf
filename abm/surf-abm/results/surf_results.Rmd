...
setwd("C:/Code/surf/abm/surf-abm/results") # This is only required when running directly in R(Studio)
...
# Select a few agents at random to plot
...
for (id in agent.ids) {
  ...
  for (activity in active.acts) {
    ...
    abline(v=acts[which(acts$Activity==activity & acts$CurrentActivity==1 
                          & dplyr::lag(acts$CurrentActivity, length(active.acts)*length(all.ids)) != 1 & acts$Agent==id),]$Iterations, col=i, lwd=2)
