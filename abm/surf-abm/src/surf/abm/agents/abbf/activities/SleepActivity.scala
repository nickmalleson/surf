    ...
    with Serializable
{
  ...
  override def performActivity(): Boolean = {
    ...
    currentAction match {
      case _ : Initialising => {
        Agent.LOG.debug(this.agent, s"is initialising SleepActivity")
        ...
        if (this.place.location.equalLocation(this.agent.location())) {
          Agent.LOG.debug(this.agent, "is at home. Starting to sleep")
          currentAction = Sleeping() // Next iteration the agent will start to sleep
        }
        else {
          Agent.LOG.debug(this.agent, "is not at home. Travelling there.")
      ...
      case _ : TravellingHome => {
        if (this.agent.atDestination()) {
          Agent.LOG.debug(this.agent, "has reached home. Starting to sleep.")
          currentAction = Sleeping() // Next iteration the agent will start to sleep
        }
        else {
          Agent.LOG.debug(this.agent, "travelling home.")
          agent.moveAlongPath()
        }
      } // TravellingHome

      case _ : Sleeping => {
        Agent.LOG.debug(this.agent, "is sleeping")
        return true
      }  // Sleeping
