...
object ABBFAgentLoaderOtleyPRIME {
  ...
  def createAgents(state: SurfABM): Unit = {
    ...
    val targetYear = SurfABM.conf.getDouble(SurfABM.ModelConfig+".Year") // The year we want to model
    ...
    for ((lineStr, i) <- Source.fromFile(flowFilename).getLines().zipWithIndex) {
      ...
      // "<blank>","OriginOA","DestinationOA","Year","FlowComm","FlowNonComm"
      if (i == 0) {
        // Check the header is as expected.
        if (line.size != 6) {
          throw new Exception(s"Problem reading the commuting file($flowFilename) - should have 6 columns but found ${line.size}: '$line'")
        }
      }
      else {
        // Ignore lines that don't represent the year we want to model
        val year: Integer = line(3).replace("\"", "").toInt
        if (year==targetYear) {
          lineCounter+=1
          ...
          val commuters: Int = math.ceil(line(4).toInt * sample).toInt
          val noncommuters: Int = math.ceil(line(5).toInt * sample).toInt
          LOG.debug(s"Origin: '$orig', Destination: '$dest', Year: `$year' Commuters: '${line(4).toInt}', Non-Commuters: '${line(5).toInt}'")
    ...
    LOG.info(s"Have read $lineCounter lines and created ${SurfABM.agentGeoms.getGeometries.size()} agents")
