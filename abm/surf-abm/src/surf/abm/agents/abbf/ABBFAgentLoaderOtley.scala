...
object ABBFAgentLoaderOtley {
  ...
  def createAgents(state: SurfABM) = {
    ...
    val filename = "./data/" + dataDir + "/oa_flows-study_area_test10.csv" // Testing with 10 commuters
    LOG.info("ABBFAGENTLOADEROTLEY is temporarily only createing a few agents")
