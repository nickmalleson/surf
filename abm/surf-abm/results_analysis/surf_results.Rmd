...
# Find the activity with the highest intensity. That needs to be plotted first
all.acts <- unique(activities$Activity)
...
for (activity in all.acts) {
  if (i==1) { # First plot
    plot(acts[acts$Activity==activity,]$date, acts[acts$Activity==activity,]$Intensity, 
         type='o', col=i, ylim=c(0,max(acts$Intensity)),
